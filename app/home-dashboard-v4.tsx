...
type Training={title:string|null;distance_m:number|null;duration_seconds:number|null;avg_hr:number|null;tss:number|null;training_load:number|null;started_at:string};
...
export default function HomeDashboardV4(){
  ...
  async function load(uid:string){
    const [p,m,s,t,g,i]=await Promise.all([
      ...
      supabase.from('training_sessions').select('title,distance_m,duration_seconds,avg_hr,tss,training_load,started_at').eq('athlete_id',uid).order('started_at',{ascending:false}).limit(1).maybeSingle(),
    ...
    setProfile(p.data as Profile|null);setMetric(m.data as Metric|null);setScore(s.data as Score|null);setTraining(t.data as Training|null);setGlucose(g.data as Glucose|null);setIntegrations((i.data as Integration[]|null)??[]);setLastRefresh(new Date());
  ...
  const stravaSync=integrations.find(x=>x.provider.toLowerCase()==='strava')?.last_synced_at;
  ...
  return <section className="home-dashboard-v4">
    ...
    <section className="hd4-readiness"><div><span className="hd4-kicker">TU ESTADO ACTUAL</span><div className="hd4-score"><strong>{readiness??'—'}</strong><em>/100</em></div><b className="hd4-green">{readiness==null?'DATOS PENDIENTES':readiness>=75?'LISTO PARA ENTRENAR':readiness>=55?'DÍA CONTROLADO':'RECUPERACIÓN PRIORITARIA'}</b><p>{readiness==null?'Peppe está esperando métricas recientes para calcular tu estado.':'Peppe prioriza lo que realmente necesitas hacer ahora.'}</p></div><div className="hd4-gauge"><div className="hd4-arc"><i style={{transform:`rotate(${readiness==null?-55:Math.max(-55,Math.min(55,(readiness-50)*1.1))}deg)`}}/></div><div className="hd4-metrics"><div><span>♡ FC REPOSO</span><strong>{metric?.resting_hr_bpm??'—'}</strong><small>bpm</small></div><div><span>⌁ HRV</span><strong>{metric?.hrv_ms??'—'}</strong><small>ms</small></div><div><span>☾ SUEÑO</span><strong>{fmtSleep(metric?.sleep_minutes??null)}</strong></div></div></div></section>

    <div className="hd4-section-title"><strong>HOY</strong><span>· {fmtWeekday()}</span></div>
    <section className="hd4-grid2"><article className="hd4-card hd4-training"><span className="hd4-status">● ENTRENAMIENTO {training?'REGISTRADO':'PENDIENTE'}</span><strong className="hd4-big">{training?.distance_m?`${(training.distance_m/1000).toFixed(2)} km`:'—'}</strong><p>{training?.title||'Sin actividad reciente importada'}</p><div className="hd4-four"><div><span>RITMO PROM.</span><strong>{fmtPace(training)}</strong><small>/km</small></div><div><span>FC PROM.</span><strong>{training?.avg_hr??'—'}</strong><small>bpm</small></div><div><span>TSS</span><strong>{training?.tss??'—'}</strong></div><div><span>CARGA</span><strong>{training?.training_load??'—'}</strong></div></div><Link href="/history">Ver análisis completo <b>›</b></Link></article>
    ...
    <section className="hd4-recovery"><span className="hd4-kicker">RECUPERACIÓN</span><div className="hd4-recovery-grid"><div><span>◯ GLUCOSA</span><strong>{glucose?.glucose_mg_dl??'—'}</strong><small>{glucose?`mg/dL ${glucose.trend||''}`:'Sin lectura reciente'}</small></div><div><span>◯ FUENTE</span><strong>{connected.has('strava')?'Strava':'—'}</strong><small>{stravaSync?`Última sync ${new Date(stravaSync).toLocaleString('es-CL')}`:'Sin sincronización reciente'}</small></div><div><span>▯ ESTADO</span><strong>{lastRefresh?'ACTUALIZADO':'—'}</strong><small>Refresco automático</small></div><p>Peppe ya no mostrará valores de ejemplo como si fueran datos actuales.</p></div></section>
