...
export default function BackofficeShell({children,title,subtitle}:{children:ReactNode,title:string,subtitle:string}){
  ...
  if(state==='denied') return <main className={styles.shell}><section className={`${styles.card} ${styles.gate} ${styles.danger}`}><span className="eyebrow">ACCESO RESTRINGIDO</span><h2>Este usuario no está autorizado.</h2><p>El backoffice no forma parte de la experiencia pública del atleta. Un Owner debe otorgar acceso explícito.</p><Link className={`${styles.button} ${styles.secondary}`} href="/">Volver</Link></section></main>;
